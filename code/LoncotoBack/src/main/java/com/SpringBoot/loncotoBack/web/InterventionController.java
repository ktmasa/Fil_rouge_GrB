...
public class InterventionController {
	...
	/*
	 * NOT WORKING
	@RequestMapping(value = "/clientsite", method = RequestMethod.GET, produces = MediaType.APPLICATION_JSON_UTF8_VALUE)
	@ResponseBody
	@CrossOrigin(origins = { "http://localhost:4200" }, methods = { RequestMethod.GET })
	public Page<Intervention> findByClientSite(@RequestParam("clientId") int idclient,@RequestParam("idsite") int idsite,@PageableDefault(page = 0, size = 5) Pageable pr) {
		return interventionRepository.findInterventionByClientSite(idsite, idclient, pr);
	}
	*/
	...
	/*
	 * @RequestMapping(value = "/next/client/{id:[0-9]+}", method =
	 * RequestMethod.GET, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	 * 
	 * @ResponseBody
	 * 
	 * @CrossOrigin(origins = {"http://localhost:4200"}, methods =
	 * {RequestMethod.GET}) public Intervention
	 * nextInterventionClient(@PathVariable("id") int
	 * id, @PageableDefault(page=0,size=15) Pageable pr){ List<Intervention>
	 * interventionList =
	 * interventionRepository.findAllWithFixedDateAfterOfClient(LocalDate.now(),
	 * id,pr).getContent(); if(interventionList == null) { return null; }else {
	 ...
	 */
	/*
	 * @RequestMapping(value = "/next/intervenant/{id:[0-9]+}", method =
	 * RequestMethod.GET, produces=MediaType.APPLICATION_JSON_UTF8_VALUE)
	 * 
	 * @ResponseBody
	 * 
	 * @CrossOrigin(origins = {"http://localhost:4200"}, methods =
	 * {RequestMethod.GET}) public Intervention
	 * nextInterventionIntervenant(@PathVariable("id") int
	 * id,@PageableDefault(page=0,size=15) Pageable pr){ List<Intervention>
	 * interventionList =
	 * interventionRepository.findAllWithFixedDateAfterOfIntervenant(LocalDate.now()
	 * , id,pr).getContent(); if(interventionList == null) { return null; }else {
	 * return interventionList.get(0); }
	 * 
	 * }
	 ...
	 * 
	 * @RequestMapping(value = "/dateorder", method = RequestMethod.GET,
	 ...
	 * findAllOrderDateReal(@PageableDefault(page=0,size=15) Pageable pr){ return
	 * interventionRepository.findAllByOrderByDateRealisation(pr);
	 * 
	 * }
	 */
